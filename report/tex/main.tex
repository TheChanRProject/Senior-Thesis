\documentclass[12pt]{report}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\graphicspath{ {../images/} }
\usepackage{setspace}
\usepackage{sectsty}

% code snippets

\usepackage{listings}
\usepackage{color}

\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}

\lstset{frame=tb,
  language=Python,
  aboveskip=3mm,
  belowskip=3mm,
  showstringspaces=false,
  columns=flexible,
  basicstyle={\small\ttfamily},
  numbers=none,
  numberstyle=\tiny\color{gray},
  keywordstyle=\color{blue},
  commentstyle=\color{dkgreen},
  stringstyle=\color{mauve},
  breaklines=true,
  breakatwhitespace=true,
  tabsize=3
}

%

\chapterfont{\fontsize{20}{18}\selectfont \centering}

\title{
{\textbf{CLASSIFYING HINDUSTANI INDIAN RAGAS WITH CONVOLUTIONAL NEURAL NETWORKS}}\\
}
\author{\textbf{BY: RISHOV S. CHATTERJEE}}
\textbf{SUBMITTED TO PITZER COLLEGE and CLAREMONT GRADUATE UNIVERSITY IN FULFILLMENT OF THE DEGREE OF BACHELOR OF ARTS IN DATA SCIENCE}
\textbf{PROFESSOR YAN LI, INFORMATION SYSTEMS AND TECHNOLOGY, CLAREMONT GRADUATE UNIVERSITY}
\textbf{PROFESSOR DAVID BACHMAN, MATHEMATICS, PITZER COLLEGE}
\date{13 May 2019}
\begin{document}
\maketitle{}

\input{gratitude/readers}

\chapter*{\centering Introduction}
\input{chapters/introduction/introduction}

\chapter*{\centering Indian Classical Ragas}
\input{chapters/ragas/ragas}

\chapter*{\centering Librosa: Processing Musical Information in Python}
\input{chapters/librosa/librosa}

\chapter*{\centering Background and Related Work}
\input{chapters/background/background}

\chapter*{\centering Methodology}
\input{chapters/methodology/methodology}

\chapter*{\centering Initial Dataset and Image Data}
\input{chapters/data/data.tex}

\chapter*{\centering Data Preparation and Processing}
\input{chapters/data_preparation_processing/data_preprocessing}

\chapter*{\centering Results and Analysis}
\input{chapters/classification-analysis/analysis}

\chapter*{\centering Future Steps}
\input{chapters/future_steps/future_steps}

\chapter*{\centering Bibliography}
\input{chapters/bibliography/bibliography}

\chapter*{\centering Appendix}
\input{chapters/appendix}

\end{document}
